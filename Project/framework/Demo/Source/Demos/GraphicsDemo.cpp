...
public:
          ...
          averageActualFPS (0)
    {
		...
		img = new  Image(Image::ARGB, 640, 480, false);

		soruce = new WebCamSource("webcam1");
#ifndef USE_DUMMY_IMAGE
		soruce->setParameter("threaded", "no");
#endif
		sink = new DisplaySink("display");
		sink->setParameter("threaded", "no");
		sink->setParameter("pull", "yes");

		soruce->addOutputElements(sink);
    ...
    void paint (Graphics& g) override
    {
		...
		if (sink->getQueueLength() > 0)
		{
			std::shared_ptr<BlockData> imgdata = sink->getData();
